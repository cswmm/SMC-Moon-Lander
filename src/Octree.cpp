...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	vector<Box> sideBoxes;
	subDivideBox8(node.box, sideBoxes);

	for (int i = 0; i < sideBoxes.size(); i++) {
		vector<int> pointsInside;
		int count = getMeshPointsInBox(mesh, node.points, sideBoxes[i], pointsInside);
		if (count > 0) {
			TreeNode child;
			child.box = sideBoxes[i];
			child.points = pointsInside;
...
bool Octree::intersect(const Box & box, TreeNode & node, vector<Box> & boxListRtn) {
	if (!node.box.overlap(const_cast<Box &>(box))) {
		return false;
	}
